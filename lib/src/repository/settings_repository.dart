...
Future<Setting> initSettings() async {
  Setting _setting;
  final String url = '${GlobalConfiguration().getValue('local_url')}DesignAPI/settings';
  print('setting url');
  print(url);
  try {
...
//   final String url = '${GlobalConfiguration().getValue('local_url')}AppUserProfile/getAppUser?app_user_id=1';
//   try {
//     final response = await http.get(url, );
//     if (response.statusCode == 200) {
//       if (json.decode(response.body)['resulat'] != null) {
//         SharedPreferences prefs = await SharedPreferences.getInstance();
//         await prefs.setString('settings', json.encode(json.decode(response.body)['result']));
//         _setting = Setting.fromJSON(json.decode(response.body)['result']);
