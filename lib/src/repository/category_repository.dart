...
Future<Stream<VendorCategory>> getCategories() async {
  Uri uri = Helper.getUri('Category/getCategory');
  ...
  try {
   ...
   return streamedRest.stream.transform(utf8.decoder).transform(json.decoder).map((data) => Helper.getData(data)).expand((data) => (data as List)).map((data) {
      // print('dws'+VendorCategory.fromJSON(data).catedes);
      return VendorCategory.fromJSON(data);
    });
    
  } catch (e) {
    print(CustomTrace(StackTrace.current, message: uri.toString()).toString());
    return new Stream.value(new VendorCategory.fromJSON({}));
  }
}
