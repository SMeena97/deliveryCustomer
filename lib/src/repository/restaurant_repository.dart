...
Future<Stream<Restaurant>> getNearRestaurants(String id) async {
  HomeController _con;
  final String _charactersPath = 'Category/getCategoryShopBased';
...
Future<Stream<Offer>> getOffers() async {
  ...
  Uri uri = Helper.getUri('Coupon/loadCoupons');
  ...
  try {
    ...
    print(e.toString());
    print(CustomTrace(StackTrace.current, message: uri.toString()).toString());
    return new Stream.value(new Offer.fromJSON({}));
  }
}
...
Future<Stream<Restaurant>> searchRestaurants(
  ...
  // Uri uri = Helper.getUri('api/restaurants');
  print(search);
  final String _charactersPath = 'Searchbox/searchPickupPoint';
  /*Map<String, dynamic> _queryParams = {};
...
Future<Stream<Restaurant>> getRestaurant(String id, Address address) async {
  final String _charactersPath = 'UserProfile/getPickupPoint';
