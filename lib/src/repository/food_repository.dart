...
Future<Stream<Food>> getTrendingFoods(Address address) async {
  Uri uri = Helper.getUri('HomePage/getPopularItems');
...
Future<Stream<Food>> getFood(String foodId,String restaurantId) async {
   final String _charactersPath = 'Items/getItemsUserBased';
  print('get food inputs..');
  print(foodId);
  print(restaurantId);
  ...
  Uri uri = Helper.getUri(_charactersPath);
  uri = uri.replace(queryParameters: _queryParameters);
  print('ppp'+uri.toString());
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', uri));
...
Future<Stream<Food>> searchFoods(String search, Address address) async {
  ...
  print(search);
  final String _charactersPath = 'Searchbox/searchItems';
  /*Map<String, dynamic> _queryParams = {};
  _queryParams['search'] = 'name:$search;description:$search';
  _queryParams['searchFields'] = 'name:like;description:like';
  _queryParams['limit'] = '5';
...
Future<Stream<Food>> getFoodsOfRestaurant(String restaurantId) async {
  ...
  // }
  uri = uri.replace(queryParameters: _queryParameters);
  print('uri123'+uri.toString());
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', uri));
 
    return streamedRest.stream.transform(utf8.decoder).transform(json.decoder).map((data) => Helper.getData(data)).expand((data) => (data as List)).map((data) {
      print("ii"+Food.fromJSON(data).toString());
      return Food.fromJSON(data);
      
    // return streamedRest.stream.transform(utf8.decoder).transform(json.decoder).map((data) => Helper.getData(data)).expand((data) => (data as List)).map((data) {
    //   print('entered...');
    //   print(data);
    //   var temp=Food.fromJSON(data);
    //   print('temp');
    //   print(temp);
    //  Food.fromJSON(data);
    });
