...
class _ShoppingCartButtonWidgetState extends StateMVC<ShoppingCartButtonWidget> {
  ...
  Widget build(BuildContext context) {
    return FlatButton(
      onPressed: () {
        print('latt'+currentUser.value.lat);
        print('lngg'+currentUser.value.lng);
        print('authss'+currentUser.value.auth.toString());
        if (currentUser.value.auth) {
          if(currentUser.value.lat!="" && currentUser.value.lng!=""){
